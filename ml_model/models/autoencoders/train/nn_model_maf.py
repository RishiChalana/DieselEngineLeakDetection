...
df_new=df_new[["rpm","fuel_rate","MAP","MAF","turbo_speed"]]
...
output_layer=Dense(units=5,activation="linear")(layer_5)
...
jb.dump(
    ...
    os.path.join(SAVE_DIR, "nn_model_preprocessing_maf.pkl")
)

autoencoder.save(
    os.path.join(SAVE_DIR, "nn_model_maf.keras")
)
...
leak_X = leak_df[ [
        "rpm","fuel_rate","MAP","MAF","turbo_speed"
    ]].values
